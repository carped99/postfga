...
static FgaL1Cache* l1_cache(void)
{
...
static bool
l1_lookup(FgaL1Cache* cache, const FgaAclCacheKey* key, uint16_t cur_generation, uint64_t now_ms, bool* allowed_out)
{
    bool found;
...
static bool l2_lookup(const FgaL2Cache* cache, const FgaAclCacheKey* key, uint64_t now_ms, bool* allowed_out)
{
    bool found;
    FgaAclCacheEntry* entry;
...
static void l2_store(FgaL2Cache* cache, const FgaAclCacheKey* key, uint64_t now_ms, uint64_t ttl_ms, bool allowed)
{
    FgaAclCacheEntry* entry;
    bool found;
    ...
    {
        LWLockRelease(cache->lock);
        ereport(ERROR, (errcode(ERRCODE_OUT_OF_MEMORY), errmsg("postfga: could not allocate L2 cache entry")));
    }
